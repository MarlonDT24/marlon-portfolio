...
export default function ContactManager() {
  ...
  // Inicializar Cal.com (Calendario)
  useEffect(() => {
	  (async function () {
		const cal = await getCalApi({});
		cal("ui", { theme: "dark", styles: { branding: { brandColor: "#22c55e" } } });
	  })();
	}, []);
  ...
  return (
     <>
      <button
        ...
        className="px-5 py-2 text-md font-medium bg-green-9 00 text-white transition-all border-2 border-green-500 rounded-full hover:bg-green-500 hover:text-white shadow-[0_0_20px_rgba(34,197,94,0.2)] hover:shadow-[0_0_30px_rgba(34,197,94,0.6)]"
      >
        Contacta Conmigo
      </button>
            ...
            <div className="p-6 overflow-y-auto custom-scrollbar bg-[#0a0a0a]">
                ...
                {activeTab === "form" && (
                    !showSuccess ? (
                        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
                            <div>
                                <h3 className="text-2xl font-bold text-white">Hablemos de tu proyecto</h3>
                                <p className="text-white/50 text-sm mt-1">Rellena el formulario y te responderé lo antes posible.</p>
                            </div>
                            
                            {/* Input Nombre */}
                            <div className="group">
                                <div className="relative">
                                    <User className="absolute top-3.5 left-4 text-green-500 group-focus-within:text-white transition-colors" size={20} />
                                    <input 
                                        {...register("username")}
                                ...
                                {errors.username && <span className="text-red-400 text-xs ml-1">{errors.username.message}</span>}
                            </div>

                            {/* Input Email */}
                            <div className="group">
                                <div className="relative">
                                    <Mail className="absolute top-3.5 left-4 text-green-500 group-focus-within:text-white transition-colors" size={20} />
                                    <input 
                                        {...register("email")}
                            ...
                            <div className="group">
                                <div className="relative">
                                    <MessageSquare className="absolute top-3.5 left-4 text-green-500 group-focus-within:text-white transition-colors" size={20} />
                                    <textarea 
                                        {...register("message")}
                        ...
                        <div className="flex flex-col items-center justify-center h-60 text-center animate-in fade-in zoom-in duration-300">
                            ...
                            <p className="text-white/60 mt-2 max-w-xs mx-auto">Gracias por contactar, Marlon. Te responderé en breve.</p>
                        </div>
                    )
                )}
                ...
                {activeTab === "calendar" && (
                    <div className="h-full min-h-112.5 w-full bg-white/5 rounded-xl overflow-hidden border border-white/5">
                        <Cal 
                            namespace="15min"
                            calLink="marlon-torres/15min" // <--- ⚠️ CAMBIA ESTO POR TU LINK REAL DE CAL.COM
